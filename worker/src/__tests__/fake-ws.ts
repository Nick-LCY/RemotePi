...
//
// We don't construct `WebSocketPair` here (that's a runtime primitive
// only available on the Workers runtime); instead the Room is exercised
// by calling its `fetch` / `webSocketMessage` / `webSocketClose` /
// `webSocketError` methods directly with the FakeWebSocket's
// `accept()`-returned handle.
...
export class FakeWebSocket {
  ...
  /** Handlers Room registers via `addEventListener`. We use a typed
   *  `Map` keyed by event name so `removeEventListener` works for
   *  symmetry (Room doesn't currently call it, but the real type
   *  signature includes it). */
  private readonly handlers = new Map<string, Set<EventListenerLike>>();

  /** The Room only consumes `string` payloads (the `webSocketMessage`
   *  path rejects binary frames immediately), so we don't model binary. */
  readyState: 0 | 1 | 2 | 3 = 0;

  /** Optional annotation some tests use to track the role the Room is
   *  expected to assign to this socket. Doesn't affect Room behaviour
   *  — the real role comes from the `X-RemotePi-Role` header on the
   *  upgrade Request. */
  role: Role | null = null;

  /** If set, the next `send()` call throws the supplied error (then
   *  resets to null). Used by the "tick send throws" spec to exercise
   *  the per-connection exception isolation in `tickHeartbeat`. The
   *  one-shot shape mirrors a real socket under transient I/O failure. */
   ...
   *  register the socket with the runtime; on the FakeWebSocket it's
   *  a no-op marker. Returns a `WebSocket`-typed reference for callers
   *  (Room's `fetch`) that pass it straight through. */
  accept(): WebSocket {
    this.readyState = 1;
    // The cast is the seam: the FakeWebSocket implements the full
    // surface Room actually exercises, but TypeScript's `WebSocket`
    // lib type carries dozens of methods we don't need. Cast at the
   ...
   *  only consumes string frames, so we serialise here — tests that
   *  need raw frame control can pass a string directly. */
   ...
   *  simulate the runtime telling us the socket died (vs. us calling
   *  close() ourselves). */
  ...
  /** Fire an error event (followed by a close in real life; tests that
   *  need the close must call `fireClose` separately). */
  fireError(): void {
    this.dispatch('error', { message: 'fake ws error' });
  }

  /** Reset all captured state — useful between tests in a shared setup
   *  helper. Doesn't reset `role` / `sendThrow` (those are per-test
   *  configuration). */
  resetCapture(): void {
 ...
 *  for closes, and the test helper dispatches those fields directly. */
