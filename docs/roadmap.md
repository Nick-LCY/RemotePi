...
| M3 | 单 session 信息往来 | bridge 对固定工作目录自动恢复 / 新建最新 session（不做选择 UI）；prompt → 流式 delta → agent_settled 全链路；abort；历史回看（`get_messages`）；`agent_settled` 后 5 分钟 idle kill；崩溃恢复；扩展 UI 阻塞对话框转发为 web 弹窗 | 浏览器完成"发消息 → 流式输出 → 任务完成"闭环；F5 刷新后状态恢复；abort 生效；空闲 5 分钟 pi 进程被 kill；select / confirm / input / editor 弹窗可交互且 agent 行为符合预期 — ✅ 完成（2026-09-08，用户裁定关单任务 08；线上部署验证随本次 CD push 确认） |
| M4 | 工作目录与 session 切换 | 工作目录列表、session 列表 / 新建 / 恢复 / 切换（`new_session` / `switch_session`）及对应 UI | 可列出目录与 session；新建、切换、恢复全链路可用 |
| M5 | UIUX 优化轮 | web 渲染结构化（markdown + thinking/tool 折叠）+ textarea 升级 + 持久侧边栏 + token localStorage + Tailwind v4 + 移动端适配 | markdown 渲染正确 / thinking-tool 默认折叠 / sidebar 双 tab / token 弹窗两模式 / 移动端抽屉焦点陷阱 / e2e 9 spec × 2 全绿 — ✅ 完成（2026-09-11 第一块 3/3 done + 2026-09-12 第二块 5/5 done，10 个 web commit + 用户并行 ADR-0013 bridge 修复，2026-09-22 用户口头确认全部完成；单测基线 977 / 全仓 1502 / 集成 32 / e2e 9 spec × 2） |
| M6 | Web UI 全量视觉重做（reference 蓝本） | 全局 accent 切换 reference 冷蓝 `#245bc4` 全家族；styles.css token 13 → 20+ + WCAG AA 校色；引入 lucide-react 单一图标库；~1280 行 styles.css 全量迁 Tailwind utilities；AppShell 双 elevated 卡 + sidebar 300px + brand 双行块；MessageList 完整气泡化 + AI 头像 + 时间戳；TokenModal / 4 类 DialogHost / DirectoryBrowser / SessionStatusBar / ChoicePanels / RecoveryView reference 形态化；暗色对比度 WCAG AA 单测 + focus 可及性 | reference accent 视觉一致 / Sidebar brand 双行块 / MessageList 气泡布局 / 弹窗分色 / 暗色对比度 AA / 单测 workspace 977 不回归 + ≥25 / 集成 32 / e2e 9 spec × 2 全绿 / build 实测体积对比基线（D8 不设门槛） — ✅ 实施收官 11/11（2026-09-22 T01-T11 + 3 个 M5 后注释清理 chore + 14 个 M6 commit `63eceb1..ab10345` = **17 commits** 领先 origin/main；T10 / T11 全量验证轮全绿；workspace **1117 tests** 终态 / 集成 32 / e2e 9 spec × 2 全绿（19.3-19.1s）/ build entry 300.07/84.86 + CSS 35.53/7.48；**待用户 push + 端到端验收**） |
