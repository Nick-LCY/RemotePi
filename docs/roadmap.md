...
| ID | 标题 | 一句话目标 | 验收方向 |
|----|------|----------|---------|
| M1 | 基础设施基座 | monorepo 脚手架（pnpm workspaces：`packages/bridge`、`packages/web`、`packages/shared`、`worker/`、`infra/`）+ shared 隧道信封类型雏形 + Terraform 管理 CF（域名 / Pages / Worker 路由）+ GitHub Actions CI 骨架 | `pnpm -r build` 通过；`terraform plan/apply` 可管理 CF 资源；CI 跑 lint/test；`wrangler dev` 本地可起 worker |
