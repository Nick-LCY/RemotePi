...
- 决策：
  ...
  5. **同会话双写是 M3 已接受后果**：若 bridge pi 正在运行，用户又在终端启动交互 pi 并写同一 session，两个进程可能同时写同一份 jsonl。M3 单用户场景以行为约定规避；M4 多 session 管理可增加“会话被外部占用”检测。
...
- 影响：
  ...
  - 同一 session 的并发写入可能造成 jsonl 双写风险。M3 接受该风险并依靠单用户行为约定，M4 预留占用检测。
...
- M4 候选：检测共享模式下 session 是否被外部进程占用，以降低同会话双写风险。
