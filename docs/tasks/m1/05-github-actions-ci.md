...
## 完成标准
- [ ] `.github/workflows/ci.yml`：`on: [push, pull_request]`；`actions/checkout@v4`；`pnpm/action-setup@v4`（读根 `package.json` 的 `packageManager` 字段，不传 version）；`actions/setup-node@v4` with `node-version-file: .nvmrc` + `cache: pnpm`；步骤 `pnpm install --frozen-lockfile` → `pnpm run lint` → `pnpm run typecheck` → `pnpm run test` → `pnpm run build`；单 job，ubuntu-latest
