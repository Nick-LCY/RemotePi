...
- [ ] `infra/data.tf`：`data "cloudflare_zone" "main" { name = "sankabox.com" }`
- [ ] `infra/dns.tf`：`cloudflare_record`，name=`remote-pi`，zone_id 用 `data.cloudflare_zone.main.id`，type=`A`，content=`192.0.2.1`，proxied=true，ttl=1（auto）
- [ ] `infra/worker_route.tf`：`cloudflare_worker_route`，pattern=`remote-pi.sankabox.com/*`，script_name=`remotepi-hello`（与 `worker/wrangler.toml` 的 name 对齐）
