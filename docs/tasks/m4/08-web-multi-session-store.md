...
- **stem 回填 watcher**（钉子 5 第 4 时机）— 监听 `session_state{session: <stem>}` 入站（来自 bridge 任务 06 pending → 真实 stem 迁移后广播）→ 任务 07 留的 stub 消费 + 任务 08 落地连带触发 level2 重查（pending → 真实 stem 时一次 refresh）——与「退回 level2」「workDir 变化」三路合并为同一 `useEffect` 依赖。**桶迁移**（R5 修复）：pending 桶（`'new'` 键）收到第一个 `session_state{session: <stem>}` 入站时，bucket 字段整体迁移（`messages` / `streamingDraft` / `queue` / `sessionPhase` / `blockedOn` / `workDir` / `recovery` / `sessionList` / `_draftHasDelta` 9 字段全量 move），迁移后 `'new'` 桶清空 + `<stem>` 桶可见；`migratePendingBucket` 防御（迁移时若 `<stem>` 桶已存在则合并而非覆盖——防极端竞态「同一 session 在两处进入」）。**时序语义固化（2026-09-09，验收期 3rd gap 修复落档）**——同步 `setCurrentSessionKey(<work_dir>)` 必须先于 `sendSessionList(<work_dir>)`（避免发出去时 `_currentSessionKey` 还是 `'new'` 触发出站字段错配），以及两个已知良性副作用：(a) 派生前发出去的 `session_list{session:'new'}` 回执会复活一个空 `'new'` 桶（属于"watcher 先于派生完成"的副作用）；(b) `'new'` 桶字段在派生前是空的，`migratePendingBucket` 5 字段判据（`messages` / `queue` / `streamingDraft` / `sessionPhase` / `blockedOn` 任一非空即不整桶覆盖）天然豁免；两者已在 `3514919` commit 落地并经 web 顺序断言钉桩（`W8.1` 钉 `setCurrentSessionKey` → `sendSessionList` 顺序，迁移防御对齐 5 字段契约）。勘误链与三层 commit `8d8e5e5` / `fe897c8` / `3514919` 修复详情见 [[tasks/m4/06-bridge-session-layer.md#勘误注记2026-09-09验收期-3rd-gapsessionjsonlpath-丢失--派生绑错-race3-commit-链|任务 06 勘误注记（3rd gap）]]。

**验收期 4th gap（2026-09-09 落档，commit `cb8a724`，本地未 push）——流式打字机丢失**。**根因链**：3rd gap 修复 (`3514919` 快照法) 让 `pending → <stem>` 迁移**稳定成功**后，stem 回填时机从"迁移失败空回"前移到 turn 开头，**首次暴露**一个一直存在但从未被触发的流程缺口——`gateForSession(<stem>)` 在 store 中 miss（只有 `'new'` 桶的 ready gate），App 据此判定 `<stem>` 是"未加载"session → 触发恢复仪式 → `ChatView` 卸载 → 流式 `message_update` delta 无人渲染 → `message_end` 收敛一次性出现 → 用户报告"打字机效果没了"。**修复四点（commit `cb8a724`）**：

1. **gate take-and-set**——watcher 增 `onRefill` seam（`<stem>` 桶就绪时触发）；App 端把 `'new'` 桶的 ready gate **移交**到 `<stem>` 键（迁移而来的 stem 历史为空，与 `session==='new'` 走 `createReadyGate` 同理无需仪式），`ChatView` 全程挂载接收流式 delta。
2. **`sendSessionList` 推迟**——watcher 的 level2 重查从"stem 回填瞬时触发"延后到 `agent_settled` 或 3s debounce（bridge 全目录内容解析 ~309ms 不再落在 turn 开头抢带宽）。
3. **顺手修 `useWsState` useCallback memoization 冻结首帧 selector 的潜在 bug**——此前被 `ChatView` 卸载掩盖（卸载 → 重挂后 useCallback 重算 selector，bug 不显形）；本轮 `ChatView` 全程挂载后该 memoization 冻结首帧 selector 会让首帧拿到陈旧数据 → 顺手补修。
4. **e2e (a) 补流式连续性断言**——`chat-view` 持续可见（无卸载）+ `recovery-in-flight` 从未出现（无仪式重发）；reviewer 推演确认该断言组合能抓本回归（单看 chat-view 持续可见会被旧实现侥幸通过——既无卸载也无仪式但流式依然丢；新增 `recovery-in-flight` 从未出现反向断言钉死"无仪式"路径）。

**数字**：单测 **674 → 688**（+14：W11 系列 7 条 + W11b 系列 7 条）/ 集成 **32**（零回归）/ **e2e 8/8 全绿 × 2** / typecheck / lint / build 全绿；web build **253.62 → 254.40 KB**（+0.78 KB，新增 +1.49 KB 减 watcher + stub 削字节 0.71 KB 净余——见 `cb8a724` diff）。

**review 结论（2026-09-09）**：0 Critical / **4 Warning**（加固级——非本轮必修）：**W1** deferred listener JSDoc 注释与重构建议 / **W2** deferred listener 加 session 过滤防极端并发 pending 误触发（**已登记 TODO / 阻塞挂账**——见下）/ **W3** `useRecoveryGateMap` 加 invariant 防双重 create / **S2-S5** 注释与重构建议集合 / **S6** e2e 轮询改 `MutationObserver`（**已登记 TODO / 阻塞挂账**——见下）；**6 Suggestion** 全部登记不本轮修。

**顺带发现的潜在地雷（登记挂账）**：`session_list` **不带 work_dir** 时 bridge 走"全 work_dir 扫描 + 全文件内容解析"路径——实测 199 文件 / 939.5MB ≈ **9.4s 同步阻塞**；当前 web 总是带 work_dir 不触发，属"潜在地雷"（任何未来 CLI / 第三方消费场景不带 work_dir 都会踩）。**并入既有"session_list 按需加载 / 缓存 / 摘要落盘"挂账条目**——见 [[current-state.md#todo--阻塞|TODO / 阻塞]]。

**教训（与本任务 3rd gap / 任务 06 三轮验收期缺口同类铁律扩展——"修 bug 会移动时序，时序变化会暴露被掩盖的下一个缺口"）**——`ChatView` 卸载是任务 08 一直存在的流程缺口：M3 时代 `chat-view` 只服务单一 session，卸载路径仅在 F5 触发，恢复仪式无脑重启 ChatView 是合理设计；M4 任务 08 引入 `SessionBucket` 分桶 + stem 回填 watcher 后，理论上同会话恢复**不应**走仪式（迁移而来的 stem 历史已在 bucket 里），但实施期未在 watcher 路径上做"已就绪 gate 移交"——3rd gap 修复前该缺口被"迁移经常失败不回填"掩盖（缺口在但路径不触发），3rd gap 让迁移稳定成功 = 路径首次触发 = 缺口显形。**与 [[tasks/m4/06-bridge-session-layer.md#勘误注记2026-09-09验收期|任务 06 验收期三缺口]]合并总结**——验收期连环缺口的共同模式：**每修一个 gap 都会改变后续流程的时序触发条件，下一个被掩盖的缺口就被推到了台面**（1st `forward compat` 占位被真实数据戳穿 / 2nd 窗口语义 chunk 级偏差被 ≥64KB 文件戳穿 / 3rd 类型层丢字段被快照法派生稳定后戳穿 / 4th gate 移交缺失被 3rd 修复让回填稳定触发戳穿）。**工程流程层对策**——M+ 候选：**任何"原本未触发"的流程路径一旦被某次修复首次激活，必须强制做一次完整链路回归（端到端断言而非单点断言）**，不能依赖 reviewer 推演（本轮 4th 即 reviewer 推演后才补 e2e 完整断言组合）。
